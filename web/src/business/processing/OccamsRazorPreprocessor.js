...
export var occam_filter = function (peptides_to_proteins, proteins_to_peptides) {
    ...
    for (var [pep, prots] of peptides_to_proteins.py_items()) {
        var is_unique = false;
        if (len(prots) ==
            1) var is_unique = true; else {
            var exact_duplicates = set();
            for (var prot1 of prots) for (var prot2 of prots) if (prot1 != prot2) if (proteins_to_peptides[prot1] == proteins_to_peptides[prot2]) {
                exact_duplicates.add(prot1);
                exact_duplicates.add(prot2)
            }
            if (exact_duplicates == prots) var is_unique = true
        }
        if (is_unique) unique_peptides.add(pep)
    }
    for (var [prot2, peps2] of proteins_to_peptides.py_items()) {
        var has_unique = false;
        for (var pep_test of peps2) if (__in__(pep_test, unique_peptides)) var has_unique = true;
        if (!has_unique) {
            var other_proteins =
                set();
            for (var pep_test2 of peps2) for (var other_prot of peptides_to_proteins[pep_test2]) if (!__in__(other_prot, remove_proteins)) other_proteins.add(other_prot);
            for (var prot3 of other_proteins) if (proteins_to_peptides[prot2].issubset(proteins_to_peptides[prot3])) if (proteins_to_peptides[prot2] != proteins_to_peptides[prot3]) remove_proteins.add(prot2)
        }
    }
