    ...
    for (var protein_group_id of protein_groups.py_keys()) {
        ...
        while (len(protein_list) > 0) {
            ...
            for (var compare_protein of protein_list) {
                var peptide_set_2_compare = protein_peptide_dict[compare_protein];
                if (peptide_set_1_group.issubset(peptide_set_2_compare)) {
                    let add = true;
                    if (!occam_flag) for (let other_protein of protein_list) if (other_protein !== compare_protein) if (!__in__(other_protein, list_of_proteins_to_remove)) {
                        let peptide_set_3_other = protein_peptide_dict[other_protein];
                        if (
                            peptide_set_1_group.issubset(peptide_set_3_other) &&
                            !peptide_set_2_compare.issubset(peptide_set_3_other) &&
                            !peptide_set_3_other.issubset(peptide_set_2_compare)
                        ) {
                            add = false;
                            break;
                        }
                    ...
                    if (add) {
                        ...
                        console.log("Adding in 1: " + compare_protein);
                        list_of_proteins_to_remove.add(compare_protein);
                    }
                } else if (peptide_set_2_compare.issubset(peptide_set_1_group)) {
                    let add = true;
                    if (!occam_flag) for (let other_protein of protein_list) if (other_protein !== compare_protein) {
                        let peptide_set_3_other = protein_peptide_dict[other_protein];
                        if (peptide_set_2_compare < peptide_set_3_other) if (!(peptide_set_2_compare < peptide_set_3_other) && !(peptide_set_3_other < peptide_set_2_compare)) {
                            add = false;
                            break;
                        }
                    }
                    if (add) {
                        subgroup_protein_list.add(compare_protein);
                        console.log("Adding in 2: " + compare_protein);
                        list_of_proteins_to_remove.add(compare_protein)
                    }
                }
            }
            console.log(list_of_proteins_to_remove);
